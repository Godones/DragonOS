...
![image-20240909165945192](./assert/image-20240909165945192.png)
...
Aya需要使用异步运行时，通过增加一些[系统调用和修复一些错误](https://github.com/DragonOS-Community/DragonOS/pull/894)DragonOS现在已经支持基本的tokio运行时。
...
1. kprobe实现 https://github.com/DragonOS-Community/DragonOS/tree/feat-ebpf/kernel/crates/kprobe
2. rbpf运行时 https://github.com/DragonOS-Community/DragonOS/tree/feat-ebpf/kernel/crates/rbpf
3. 系统调用支持
4. helper函数支持
...
通过增加`StackFrame`结构，eBPF程序在发生函数调用时将会切换栈，同时在切换栈的过程需要保存一些寄存器信息：
...
目前对bpf to bpf call的最大层数限制为8.



### kprobe

eBPF被允许挂载到kprobe上，并可以动态地使能。kprobe新增了2个字段来解决这个问题:
...
同时，kprobe的管理数据结构被移动到内核代码树上，因为kprobe可以被动态地进行修改，这需要内核的锁数据结构支持。
...
用户程序使用kprobe通过`perf_event_open`系统调用完成， 下一节会讨论这些内容。 
...
- [x] 普通的eBPF程序(没有指定特殊的Map)
- [x] 使用Aya框架生成项目模板
- [x] 使用内核已经实现的Map的eBPF程序
- [x] 可以和kprobe配合使用
- [x] 内核Map相关的接口定义已经实现，添加新的Map较为简单
...
1. 对Aya的完整支持可以在内核生成必要的伪文件后完成
2. 对bpf()更多命令的支持需要和文件系统配合，剩余的主要命令与bpf程序绑定/信息获取相关
3. 对helper函数的添加可以按需求逐步添加
4. 支持更多eBPF程序类型需要增加新的跟踪机制: tracepoint等
